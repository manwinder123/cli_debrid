...
def prepare_manual_assignment():
    ...
    try:
        ...
        if media_type in ['tv', 'show']:
            ...
            # TMDB fallback — Trakt/TVDB are often unavailable (403 / rate-limited).
            # TMDb is configured and reliable, so fetch the show's seasons there
            # so "all" / "seasons" selections can still build target items.
            if not _covers_needed_seasons(metadata.get('seasons')) and metadata.get('tmdb_id'):
                _tmdb_seasons = _fetch_tmdb_seasons_fallback(str(metadata['tmdb_id']))
                if _tmdb_seasons:
                    metadata['seasons'] = {**dict(metadata.get('seasons') or {}), **_tmdb_seasons}
                    logging.info(
